...
- Task rows distinguish `totalElapsedSeconds` / `totalEstimatedPercent` / `averageSecondsPerPercent` from `latestTurnElapsedSeconds` / `latestTurnEstimatedPercent` / `latestTurnSecondsPerPercent`. Parent task quota totals include discovered children; parent elapsed time merges concurrent intervals without counting them twice. The latest turn includes child executions started during that turn; child rows apply the same rule to their own descendants. Say “最近一次含本轮启动的 X 个子任务” when `latestTurnChildCount` is known. Sort tasks by their own latest-turn start time, newest first, regardless of running/completed status; keep unknown start times last. Never substitute cumulative quota for a missing latest-turn estimate. Missing evidence remains `—`.
