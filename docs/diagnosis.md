...
启动成功应看到正常面板，而不是 JSON 错误、空白页或仅有连接状态条。启动器会检查 HTML、脚本与样式是否可读取；首次加载的账户数据仍可能需要一个采样周期。

- 若 API 正常但网页出现文件权限错误，先检查服务启动位置和网页文件的可读性。仅凭 `EPERM` 不能断定目录被移动。关闭页面不会停止后台服务。
- 确认是本插件的旧监控服务后，可在当前插件目录运行 `node scripts/stop.mjs`，再运行 `node server/launcher.mjs`；仓库根目录对应 `node plugins/codex-quota-monitor/scripts/stop.mjs`。不要停止其他 Codex 任务或修改应用、对话数据库。
- `compact=1` 是精简布局，应仍显示账户、任务和重置等主要面板。若旧版只显示连接条，可先去掉这个查询参数并刷新，再更新插件。保留地址中的原端口和访问凭据，不要公开带凭据的网址。
- 自动打开后，有浏览器工具时应确认面板内容确实可见，再报告完成；无法进行视觉检查时说明检查范围。
