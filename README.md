...
Codex Quota Monitor 是一个codex插件，用来观察 Codex 账户窗口、发现本机会话，并给出每会话订阅额度消耗的透明估算。它不会把估算结果包装成官方精确拆账。
...
账户和模式总览的百分比及普通小数继续显示两位；会话行和额度拆分中，正数且小于 0.1% 的估算会保留足够位数以显示至少三位有效数字，最多显示 10 位小数，极小值使用科学计数法。内部计算保留更高精度。显示精度不代表底层数据或官方拆账具有对应精度。样本不足时会显示 `—`，不会把缺少证据的速度显示成零。
...
默认 24 小时滑动窗口内的执行记录参与总耗时、额度估算和平均速度计算。窗口可自定义为 1–168 小时；扩大窗口不会恢复已经清理的观测，也不会删除 Codex 原始记录。父任务总额度包含已发现子任务的消耗；总耗时合并父子任务的执行区间，并发时间只计一次。总计覆盖统计窗口内的所有轮次；最近一次只包含最近一轮及本轮启动的子任务。最近一次会话的统计包含本轮启动的子任务，界面会显示本任务和子任务的额度拆分。会话消耗排序先显示正在执行的任务；每组内再按任务最后一次会话启动时间从新到旧。耗时来自本机保留的执行记录，可能不完整。

“最近一次会话”包含该任务自身最近一轮以及本轮启动的子任务；子任务各自单列，也按同样规则包含其本轮启动的下级任务。已完成任务使用该轮的执行时间和额度记录计算预测；正在执行的任务优先使用最近 5 秒校准的 token 估算，不代表官方在 5 秒内精确扣费，缺少校准时回退本轮均速。额度与平均速度只覆盖监控窗口；旧数据不能可靠回填，缺少样本用 `—` 表示。

账户余量趋势的右上角显示“统计自”时间，折线起点和终点直接标注起始余量与最新余量。
...
账户窗口是账户级数据。每会话数据属于估算，并且只覆盖选定统计窗口内的样本，不代表窗口外的会话历史。当前 `threadUsage=null` 时，插件只能按照本机 token 增量比例分摊账户变化；不同模型的真实权重未知，其他设备、后台任务或未被发现的线程也可能污染账户窗口，所以这类结果会标为低置信度。账户窗口发生变化而本机没有可匹配 token 增量时，差额会留在 `unattributed`，不会强行分配给某个会话。服务离线期间的账户变化也归入未归因项；同一窗口内重启保留已记录值。归因面板遇到不完整旧记录时，会从其后的完整采样开始计算，并显示实际起算时间；这不删除任务列表中的历史估算。尚未出现额度变化时显示等待状态，不将零分母伪装为归因率。
