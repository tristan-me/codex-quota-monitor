...
运行中的任务置顶，运行组与非运行组内部各自保持已保存的顺序；消耗数字和更新时间变化不会重新排序。任务转为待机后会移到非运行组，剩余运行任务留在顶部；子任务使用相同规则，基础顺序随监控状态保存。

每个任务分两行显示累计与最近一轮：

```text
任务总计耗时xx，任务总计消耗估算xx，平均每 1% 额度能撑 xx；
最近一次会话耗时xx，最近一次会话消耗估算xx，预计接下来每 1% 额度能撑 xx
```

总耗时按本机保留的执行记录累计，父任务含已发现的子任务，并发区间只计一次；每线程最多读取最近 5000 条执行记录，旧格式或缺失记录会标为部分历史。额度累计保留监控期间采到的跨重置记录，平均速度也只使用有计时和额度样本的观测时段，因此不是用全部历史耗时除以局部额度估算。

“最近一次会话”指该任务自身最近一轮执行，子任务各自单列。该轮额度独立按轮次记录；跨轮采样中无法确定归属的 token 增量不会直接记入新一轮。旧版本只有累计总账，无法可靠回填过去每一轮，未采到的数据用 `—`，不表示零消耗。待机或缺少近期样本时不显示未来速度预测。
