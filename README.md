...
任务与子会话按首次出现的顺序固定排列，新出现的任务追加到末尾。运行状态、消耗数字和更新时间变化不会重新排序；顺序随本地监控状态保存，服务重启后继续使用。

会话累计消耗保留采到的跨重置记录；没有观测的数据用 `—`，不表示零消耗。当前/最后一轮的处理时间与监控期间的活跃时间不是同一概念，平均速度只使用有计时记录的观测时段。旧版本已经清空或从未采到的数据无法自动回填。
