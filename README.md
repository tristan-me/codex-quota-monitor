...
默认 24 小时滑动窗口内的执行记录参与总耗时、额度估算和平均速度计算。窗口可自定义为 1–168 小时；扩大窗口不会恢复已经清理的观测，也不会删除 Codex 原始记录。父任务总额度包含已发现子任务的消耗；总耗时合并父子任务的执行区间，并发时间只计一次。因此首次执行时两行耗时可能相同，但总额度高于本任务最近一轮额度。界面会显示本任务和子任务的额度拆分。耗时来自本机保留的执行记录，可能不完整。
