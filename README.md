...
默认 24 小时滑动窗口内的执行记录参与总耗时、额度估算和平均速度计算。窗口可自定义为 1–168 小时；扩大窗口不会恢复已经清理的观测，也不会删除 Codex 原始记录。父任务总额度包含已发现子任务的消耗；总耗时合并父子任务的执行区间，并发时间只计一次。总计覆盖统计窗口内的所有轮次；最近一次只包含最近一轮及本轮启动的子任务。最近一次会话的统计包含本轮启动的子任务，界面会显示本任务和子任务的额度拆分。会话消耗排序按任务最后一次会话启动时间从新到旧。耗时来自本机保留的执行记录，可能不完整。
