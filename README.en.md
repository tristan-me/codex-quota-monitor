...
Use the **Monitoring source** selector at the top to switch between your **Codex account, Muse, and other custom APIs configured in the desktop app**. It initially selects the desktop's current provider and changes only the monitoring view. API views separately show recorded input, cached input, output, and total tokens. Muse also shows a reference USD estimate at official OpenAI API rates. Actual charges and balances come from the provider's records; API views do not show Codex subscription percentages.
...
| API usage and reference cost | Recorded tokens; Muse summaries, tasks, and execution tooltips show an OpenAI API USD reference estimate and priced coverage. |
...
**Muse reference USD estimates** use the short-context baseline from [official OpenAI API pricing](https://developers.openai.com/api/docs/pricing), checked on 2026-09-20. Identified models, token breakdowns, and service tiers are priced separately; mixed history does not inherit the task's latest model. Supported Fast, Priority, and Flex rates apply where available; a missing tier assumes Standard and is disclosed. This USD reference is distinct from Codex credits, subscription quota, and Muse bills or balances.

Cache-write counts and per-request context lengths are unavailable, so non-cached input uses the ordinary input rate. Estimates exclude cache-write uplift, long-context premiums, tool or region fees, and provider markup. Cached tokens are already part of input, and reasoning tokens are part of output, so neither is counted twice. Unknown models, unsupported tiers, and unclassified tokens remain unpriced. The page shows priced coverage and unpriced token counts; a partial amount represents only the priced portion.
