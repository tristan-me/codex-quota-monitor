...
The account statistics window is 24 hours by default and can be set from 1 to 168 hours; it controls account trends, attribution, and current rate calibration. Known task totals are retained independently. The UI refreshes every 5 seconds and account data every 30 seconds by default; both intervals are adjustable, and you can pause background reads while keeping access to existing records. Changing the account window does not delete Codex conversations; increasing it cannot restore previously removed samples. Very small positive estimates keep a few extra readable decimal places instead of appearing as zero.
