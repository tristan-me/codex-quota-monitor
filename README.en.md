...
Codex task rows show total consumption and mean time per 1% of quota. Charts start collapsed as small previews on the right; expand or collapse each independently, with that choice retained after refresh. Executions use different colors and are joined between runs; hover or keyboard-focus a segment for its start, end, recorded duration, consumption, and mean rate. When only execution boundaries and totals are known, intermediate progress is explicitly estimated; connections between runs do not add consumption. API charts show token consumption and duration, without quota percentages or time per 1%.
